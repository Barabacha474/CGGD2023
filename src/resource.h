...
namespace cg
{
	...
	template<typename T>
	inline resource<T>::resource(size_t size) //one dimension version
	{
		stride = 0;
		data.resize(size);
	}
	template<typename T>
	inline resource<T>::resource(size_t x_size, size_t y_size) //two dimension version
	{
		stride = x_size;
	...
	template<typename T>
	inline T& resource<T>::item(size_t item) //one dimension version
	{
		return data.at(item);
	}
	template<typename T>
	inline T& resource<T>::item(size_t x, size_t y) //two dimension version
	{
